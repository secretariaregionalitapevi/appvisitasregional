...
def administration_user_modules(request, user_id):
    ...
    try:
        if not _get_table("profiles", {"select": "user_id", "user_id": f"eq.{user_id}", "limit": "1"}):
        ...
        log_audit(request, "UPDATE_USER_MODULE_ACCESS", "ADMIN", {
            "target_user_id": str(user_id), "before": sorted(before), "after": sorted(requested),
        })
        return JsonResponse({"user_id": str(user_id), "modules": sorted(requested)})
    except requests.RequestException:
        return JsonResponse({"error": "Falha ao atualizar permissões de pasta. Verifique a migração 011."}, status=502)
