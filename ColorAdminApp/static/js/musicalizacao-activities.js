(() => {
  ...
  function launchesTable(rows, limit = null) {
    ...
    return `<div class="table-responsive"><table class="table activity-table"><thead><tr><th>Data</th><th>Município</th><th>Polo</th><th>Atividade</th><th class="text-center">Crianças</th><th class="text-center">Equipe</th><th class="text-end">Ações</th></tr></thead><tbody>${list.map(row => `<tr><td><strong>${dateLabel(row.data_aula)}</strong><span class="activity-small">${esc(row.ciclo || 'Ciclo não informado')}</span></td><td>${esc(row.cidade || '—')}</td><td><span class="activity-polo-name">${esc(row.polo || '—')}</span></td><td>${esc(row.nome_atividade || (row.numero_aula ? `Atividade ${row.numero_aula}` : '—'))}</td><td class="text-center"><span class="activity-count-pill">${number(row.meninos_presentes) + number(row.meninas_presentes)}</span></td><td class="text-center">${number(row.instrutores_presentes) + number(row.colaboradores_presentes) + number(row.coordenadores_presentes)}</td><td>${actionButtons(row)}</td></tr>`).join('')}</tbody></table></div>`;
  }
  ...
  function showDetails(row) {
    ...
    const items = [['Município',row.cidade],['Polo',row.polo],['Data',dateLabel(row.data_aula)],['Ciclo',row.ciclo],['Número da aula',row.numero_aula],['Atividade',row.nome_atividade],['Meninos',number(row.meninos_presentes)],['Meninas',number(row.meninas_presentes)],['Instrutores',number(row.instrutores_presentes)],['Colaboradores',number(row.colaboradores_presentes)],['Coordenação',number(row.coordenadores_presentes)],['Observações',row.observacoes]];
