...
O worker sincroniza aulas e chamadas por delta antes dos lotes de históricos individuais. A rotina incremental roda a cada cinco minutos, por padrão, e revisa uma janela curta de chamadas recentes. A cada aluno concluído, grava `last_history_sync_at`; depois de queda de energia ou reinício, a fila recomeça no primeiro estado ainda pendente. Reprocessamentos são idempotentes.
...
2. Configure no ambiente do computador do worker:
   ...
   - `SAM_SYNC_INTERVAL_SECONDS`: intervalo ocioso entre ciclos; padrão 120 segundos.
   - `SAM_SYNC_HISTORY_LIMIT`: históricos completos processados por lote; padrão 100.
   - `SAM_CLASSES_SYNC_INTERVAL_SECONDS`: intervalo da sincronização incremental de aulas; padrão 300 segundos.
   - `SAM_CLASSES_LOOKBACK_DAYS`: janela para revisar chamadas recentes que ainda podem mudar; padrão 14 dias.
