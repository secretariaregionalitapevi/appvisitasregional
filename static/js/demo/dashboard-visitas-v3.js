...
var handleDateRangeFilter = function() {
	moment.locale('pt-br');
	var dashboardEnd = moment('2026-08-05', 'YYYY-MM-DD');
	var dashboardStart = moment('2026-07-29', 'YYYY-MM-DD');
	$('#daterange-filter span').html(dashboardStart.format('D MMMM YYYY') + ' - ' + dashboardEnd.format('D MMMM YYYY'));
	$('#daterange-prev-date').html(moment(dashboardStart).subtract(8, 'days').format('D MMMM') + ' - ' + moment(dashboardStart).subtract(1, 'days').format('D MMMM YYYY'));
	...
	$('#daterange-filter').daterangepicker({
		...
		ranges: {
			'Hoje': [dashboardEnd, dashboardEnd],
			'Ontem': [moment(dashboardEnd).subtract(1, 'day'), moment(dashboardEnd).subtract(1, 'day')],
			'Últimos 7 dias': [moment(dashboardEnd).subtract(6, 'days'), dashboardEnd],
			'Últimos 30 dias': [moment(dashboardEnd).subtract(29, 'days'), dashboardEnd],
			'Este mês': [moment(dashboardEnd).startOf('month'), moment(dashboardEnd).endOf('month')],
			'Mês anterior': [moment(dashboardEnd).subtract(1, 'month').startOf('month'), moment(dashboardEnd).subtract(1, 'month').endOf('month')]
		},
		opens: 'right',
		drops: 'down',
